...
export default function AboutPage() {
  return (
    <div className="flex flex-col">
      ...
      {/* ── Hero ────────────────────────────────────────── */}
      <section className="mx-auto max-w-5xl w-full px-6 pt-20 pb-12 section-fade">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-8">
          {/* Photo */}
          <div className="flex-shrink-0">
            <div className="relative h-36 w-36 rounded-full overflow-hidden ring-4 ring-border shadow-xl">
          ...
          <div className="flex flex-col gap-3 text-center sm:text-left">
            ...
            <div className="flex flex-wrap justify-center sm:justify-start gap-x-5 gap-y-2 text-sm text-muted-foreground">
              <span className="flex items-center gap-1.5">
                <MapPin className="h-4 w-4" /> Footscray, VIC · Ho Chi Minh City, VN
              </span>
              <a href="mailto:phamtran2082005@gmail.com" className="flex items-center gap-1.5 hover:text-primary transition-colors link-underline">
            ...
            <div className="flex justify-center sm:justify-start gap-4">
              <a href="https://github.com/plebaotrn" target="_blank" rel="noopener noreferrer"
                ...
                <Github className="h-4 w-4" /> github.com/plebaotrn
              </a>
        ...
        <ScrollReveal>
          <div className="mt-8 max-w-2xl p-5 rounded-xl border border-border bg-muted/40">
      ...
      <section className="mx-auto max-w-5xl w-full px-6 py-12 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {skillGroups.map((group, i) => (
            <ScrollReveal key={group.label} delay={i * 70}>
              <div className="flex flex-col gap-2">
                <p className="text-xs font-semibold text-foreground">{group.label}</p>
                <div className="flex flex-wrap gap-1.5">
                  {group.items.map((item) => (
                    <span key={item} className="tag-pill px-2.5 py-1 text-xs font-mono bg-muted text-muted-foreground rounded-full border border-border">
                      {item}
                    </span>
                  ))}
                </div>
      ...
      <div className="border-t border-border bg-muted/40">
        ...
        <section className="mx-auto max-w-5xl w-full px-6 py-12 flex flex-col gap-6">
          ...
          <ScrollReveal>
            <div className="card-hover flex items-start gap-4 p-5 rounded-xl border border-border bg-card shadow-sm">
      ...
      <section className="mx-auto max-w-5xl w-full px-6 py-12 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-4">
          {experience.map((exp, i) => (
            <ScrollReveal key={`${exp.org}-${exp.period}`} delay={i * 80}>
              <div className="card-hover flex items-start gap-4 p-5 rounded-xl border border-border bg-card shadow-sm">
              ...
              <div className="flex flex-col gap-3">
                {certifications.map((cert, i) => (
                  <ScrollReveal key={cert.name} delay={i * 70}>
                    <div className="card-hover flex items-start gap-3 p-4 rounded-xl border border-border bg-card shadow-sm">
              ...
              <div className="flex flex-col gap-3">
                {languages.map((lang, i) => (
                  <ScrollReveal key={lang.name} delay={i * 70}>
                    <div className="card-hover flex items-center justify-between p-4 rounded-xl border border-border bg-card shadow-sm">
